...
public class Vips {
	 ...
	 */
	private void performSegmentation() throws FileNotFoundException, IOException
	{
		...
		//String testDirectory=_urlString.substring(7, _urlString.indexOf("keyword")+7).trim();
		String testDirectory="";
		vipsOutput.writeXML(constructor.getVisualStructure(), _viewport, _foldername, testDirectory);

		endTime = System.nanoTime();

		long diff = endTime - startTime;

		/*System.out.println("\nExecution time of ViDE: " + diff + " ns;  " +
				(diff / 1000000.0) + " ms;  " +
	 ...
	 * @throws IOException 
	 */
	public void startSegmentation(String url) throws IOException
	{
		setUrl(url);
		_urlString=url;
		startSegmentation();
	...
	public void startSegmentation() throws IOException
	{
		try
		{
			...
			{   
				...
				this._foldername=outputFolder;
				//this._testDirectory=_urlString.substring(7, _urlString.indexOf("keyword")+7).trim();
			...
			System.err.println("Failure in creating viewport");
			String[][] resultTableCSV=new String[0][0];
			String alignedItemsTable="";
			VipsOutput vipsOutput = new VipsOutput();
			vipsOutput.writeToCSV(resultTableCSV, _foldername, _testDirectory);
			//vipsOutput.writeToTXT(alignedItemsTable, _foldername, _testDirectory);
