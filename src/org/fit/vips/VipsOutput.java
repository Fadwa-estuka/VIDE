...
public final class VipsOutput {
	...
	private void writeVisualBlocks(Element parentNode, VisualStructure visualStructure)
	{
		...
		if(visualStructure.getNestedBlocks().size()!=0){
			...
			layoutNode.setAttribute("DOMCldNum", "0");
		...
		layoutNode.setAttribute("nestedBlocks", String.valueOf(visualStructure.getNestedBlocks().size()));
		
		visualStructure.setOrder(_order);
	 ...
	 * @param _foldername 
	 */
	public void writeXML(VisualStructure visualStructure, Viewport pageViewport, String _foldername, String testDirectory) throws FileNotFoundException, IOException 
	{
		try
		{
			...
			String pageTitle="";
			if(pageViewport.getRootElement().getOwnerDocument().getElementsByTagName("title").item(0)!=null)
				pageTitle = pageViewport.getRootElement().getOwnerDocument().getElementsByTagName("title").item(0).getTextContent();
			...
			DataRecordsExtractor dataRecordsExtractor=new DataRecordsExtractor(vipsElement, visualStructure);
			ArrayList results=dataRecordsExtractor.extractDataRecords("dataRegion");
			//ArrayList results=dataRecordsExtractor.extractDataRecords("wholePage");
			String alignedItemsTable="";
			String[][] resultTableCSV=new String[0][0];
			if(results.size()>0){
				alignedItemsTable=(String) results.get(0);
				resultTableCSV=(String[][]) results.get(1);	
			}
			...
			if (_escapeOutput)
			{
				...
				writeToTXT(alignedItemsTable, _foldername, testDirectory);
				writeToCSV(resultTableCSV, _foldername, testDirectory);
			}
			else
			{
	...
	/**
	 * Write to csv
	 * @param resultTableCSV
	 * @param foldername
	 * @param testDirectory
	 */
	public void writeToCSV(String[][] resultTableCSV, String foldername, String testDirectory) throws IOException{
		//BufferedWriter br = new BufferedWriter(new FileWriter(_foldername+"/data records-ViDE.csv"));
		BufferedWriter br = new BufferedWriter(new FileWriter(testDirectory+"data records-ViDE-DataRegion.csv"));
		//BufferedWriter br = new BufferedWriter(new FileWriter(testDirectory+"data records-ViDE-WholePage.csv"));
		StringBuilder sb = new StringBuilder();
		for (String[] row : resultTableCSV) {
			for(String item : row){
				sb.append(item);
				sb.append(",");
			}
	...
	/**
	 * Write to txt
	 * @param alignedItemsTable
	 * @param foldername
	 * @param testDirectory
	 */
	public void writeToTXT(String alignedItemsTable, String foldername, String testDirectory) throws IOException{
		//FileWriter fstream = new FileWriter(foldername+"/DataRecords-ViDE-DataRegion.txt");
		FileWriter fstream = new FileWriter(foldername+"/DataRecords-ViDE-WholePage.txt");
		fstream.write(alignedItemsTable);
		fstream.close();
	}
